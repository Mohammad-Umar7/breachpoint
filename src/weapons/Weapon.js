...
export class Weapon {
  ...
  _buildMuzzleFlash() {
    // A gadget belongs here with the knife and the frag: none of the three has
    // a barrel. Without it the terminal would get two additive flash quads and
    // a PointLight welded to its `muzzle` anchor — invisible until something
    // eventually set `flashTime`, and then a screen that flashes orange.
    if (this.def.melee || this.def.throwable || this.def.gadget) {
...
function buildViewModel(group, def, assets) {
  ...
  switch (def.modelClass) {
    ...
    /* ------------------------------------------------------------ gadgets */
    /*
     * The drone's field terminal: a rugged slab held flat in both hands.
     *
     * Everything lives on a tilted sub-group rather than being authored tilted
     * piece by piece, because the SCREEN ANCHOR has to share the panel's exact
     * plane. `DroneScreen` parents the feed quad to that anchor, and a quad a
     * degree off the face it is supposed to be set into reads as a sticker
     * floating over the handset.
     *
     * Parenting the quad inside this group — a grandchild of the view model's
     * holder — is also what makes it show and hide with the terminal for free:
     * the holder's visibility pass iterates DIRECT children only and skips
     * anything without `isWeaponGroup`, so a grandchild is never touched.
     */
    case 'terminal': {
      const panel = new THREE.Group();
      panel.name = 'terminalPanel';
      panel.position.set(0, -0.015, -0.02);
      // Face tilted back toward the eye, the angle you actually hold a handset
      // at. A panel square to the barrel axis is edge-on to the camera and the
      // feed on it is unreadable.
      ...
      const W = 0.28, H = 0.175, D = 0.024;
      const FACE = D / 2;
      piece(box(W, H, D), dark, 0, 0, 0);                        // body
      piece(box(W * 0.30, H * 0.86, D * 1.18), poly, -W * 0.35, 0, 0);   // left grip
      piece(box(W * 0.30, H * 0.86, D * 1.18), poly, W * 0.35, 0, 0);    // right grip

      /*
       * The bezel stands 2 mm PROUD of the body, and the screen anchor 4 mm
       * proud of that.
       *
       * Not flush, and not a coincidence: two coincident same-side faces
       * z-fight, and the result shimmers exactly where the player is being
       * asked to read a small low-resolution picture.
       */
      piece(box(0.176, 0.132, 0.003), metal, 0, 0.012, FACE + 0.002);

      // Rubber corner bumpers.
      for (const sx of [-1, 1]) {
        for (const sy of [-1, 1]) {
          piece(box(0.032, 0.030, D * 1.3), poly, sx * (W / 2 - 0.014), sy * (H / 2 - 0.013), 0);
        }
      }

      // Two thumbsticks, low on the face where the thumbs fall.
      for (const sx of [-1, 1]) {
        piece(cyl(0.011, 0.013, 0.016, 10), metal, sx * 0.104, -0.052, FACE + 0.008, Math.PI / 2);
        piece(cyl(0.019, 0.019, 0.004, 12), dark, sx * 0.104, -0.052, FACE + 0.001, Math.PI / 2);
      }

      // Stubby antenna, raked back out of the sight line.
      piece(cyl(0.0035, 0.0045, 0.085, 8), metal, 0.098, H / 2 + 0.036, -0.004, 0.34);

      // The feed goes here. Named, because DroneScreen looks it up by name and
      // a rename would silently leave the panel blank.
      const screen = new THREE.Object3D();
      screen.name = 'screen';
      screen.position.set(0, 0.012, FACE + 0.006);
      panel.add(screen);

      // A gadget can never aim, but `_sightLocal` is resolved for every weapon
      // in the constructor, so it needs somewhere sane to resolve to.
      anchor('sight', 0, 0.0, -0.05);
      anchor('muzzle', 0, 0.0, -0.09);
      anchor('eject', 0, 0, 0);
      break;
    }
